...
// Operações sobre o NodoAB

PNodoAB criarNodoAB(DadosAB);
...
PNodoAB pesquisarAB (DadosAB, PNodoAB);

void consultarAB (DadosAB, PNodoAB);

void atualizarAB (DadosAB, DadosAB, PNodoAB);

// inserir de forma que fique o mais possível equilibrada
PNodoAB inserirPorAlturaAB (DadosAB, PNodoAB);

// remover um elemento de uma árvore
//  X está na árvore binária T
PNodoAB removerAB (DadosAB, PNodoAB);
...
PNodoAB pesquisarAB (DadosAB X, PNodoAB T)
{
  PNodoAB P;
  if (T == NULL)
    return NULL;
...
void consultarAB (DadosAB X, PNodoAB T)
{
  PNodoAB P;
  P = pesquisarAB(X, T);
  if (P != NULL)
    ...
    printf("Nao existe elemento na arvore com a mesma chave de X!\n");
}

void atualizarAB (DadosAB X, DadosAB Y, PNodoAB T)
{
  PNodoAB P;
  P = pesquisarAB(X, T);
  if (P != NULL)
...
PNodoAB inserirPorAlturaAB (DadosAB X, PNodoAB T)
{
  if (T == NULL){
    T = criarNodoAB(X);
    return T;
...
PNodoAB procurarFolhaAB (PNodoAB T, DadosAB *X)
{
...
//  X está na árvore binária T
PNodoAB removerAB (DadosAB X, PNodoAB T)
{
  PNodoAB P;
